...
describe('SongListenedDetector', () => {
  ...
  const createSongState = (position: number, totalDuration: number, song: Song): Player => ({
    hasSong: true,
    isPlaying: true,
    song: {
      ...song,
  ...
  it('should trigger exactly once when threshold is reached', () => {
    ...
    const postState = createSongState(REQUIRED_TICKS + 1, TOTAL_DURATION, SONG_A);
  ...
  it('should track different songs independently', () => {
    const durationA = 100, durationB = 200;
  ...
  describe('Looped Song Handling', () => {
    ...
    it('should handle multiple consecutive loops', () => {
      expect(detector.songTick(createSongState(0, TOTAL_DURATION, SONG_A))).toBe(false);
      for (let i = 1; i < REQUIRED_TICKS; i++) {
        const result = detector.songTick(createSongState(i, TOTAL_DURATION, SONG_A));
    ...
    it('should reset progress if looping after partial playback', () => {
      ...
      for (let i = 1; i <= 50; i++) {
        expect(detector.songTick(createSongState(i, TOTAL_DURATION, SONG_A))).toBe(false);
      }
